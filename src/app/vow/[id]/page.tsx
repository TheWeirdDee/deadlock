...
import { VowReactions } from '@/components/VowReactions';
...
export default function VowPage() {
  ...
  const handleSpectate = async (prediction: boolean, customAmount?: number) => {
    ...
    await doContractCall({
      ...
      onFinish: () => {
        toast(`Bet submitted — predicting ${prediction ? 'SUCCESS' : 'FAILURE'} with ${amount} STX.`, 'success');
        setSpectatorPending(false);
        fetchData();
      },
      onCancel: () => { toast('Bet cancelled.', 'info'); setSpectatorPending(false); },
    });
  };
  ...
  const handleVote = async (success: boolean) => {
    ...
    await doContractCall({
      ...
      postConditionMode: PostConditionMode.Allow,
      onFinish: () => {
        toast(`Vote cast: ${success ? 'SUCCESS' : 'FAILURE'}`, 'success');
        setVotePending(false);
        fetchData();
      },
      onCancel: () => { toast('Vote cancelled.', 'info'); setVotePending(false); },
    });
  };

  const handleSubmitProof = async () => {
    if (!proofUrl) return;
    // Basic URL validation — block obviously invalid proof links
    const BLOCKED_HOSTS = ['localhost', '127.0.0.1', 'example.com', '0.0.0.0'];
    try {
      const parsed = new URL(proofUrl);
      if (!['http:', 'https:'].includes(parsed.protocol)) throw new Error('invalid protocol');
      if (BLOCKED_HOSTS.some(h => parsed.hostname === h || parsed.hostname.endsWith(`.${h}`))) {
        toast('Invalid proof URL — must be a public link.', 'error');
        return;
      }
    } catch {
      toast('Please enter a valid proof URL (must start with https://).', 'error');
      return;
    }
    await doContractCall({
      contractAddress: contractDetails.address,
      ...
      postConditionMode: PostConditionMode.Allow,
      onFinish: () => {
        toast('Proof submitted — adjudication window is now open.', 'success');
        setProofUrl('');
        fetchData();
      },
      onCancel: () => toast('Proof submission cancelled.', 'info'),
    });
  };
  ...
  const handleClaimFailure = async () => {
    await doContractCall({
      ...
      postConditionMode: PostConditionMode.Allow,
      onFinish: () => {
        toast('Failure settlement triggered on-chain.', 'success');
        fetchData();
      },
      onCancel: () => toast('Settlement cancelled.', 'info'),
    });
  };
  ...
  const handleFinalizeChallengedVow = async () => {
    await doContractCall({
      ...
      postConditionMode: PostConditionMode.Allow,
      onFinish: () => {
        toast('Vow finalized on-chain. Funds distributed.', 'success');
        fetchData();
      },
      onCancel: () => toast('Finalize cancelled.', 'info'),
    });
  };
  ...
  const handleAcceptRivalVow = async () => {
    ...
    await doContractCall({
      ...
      postConditionMode: PostConditionMode.Allow,
      onFinish: () => {
        toast('Rival challenge accepted — the battle is on.', 'success');
        fetchData();
      },
      onCancel: () => toast('Challenge acceptance cancelled.', 'info'),
    });
  };
  ...
  return (
    <main className="min-h-screen p-4 md:p-12 lg:p-24 max-w-7xl mx-auto bg-surface text-ink relative font-space">
      ...
      <div className="flex gap-4 mb-12 flex-wrap">
        <a href="/" className="btn-outline py-2.5 px-6 text-xs rounded-full tracking-widest font-bold border-line-strong hover:border-ink-muted bg-surface-raised hover:bg-surface-hover transition-all duration-300">
          ← BACK TO FEED
        </a>
        <button
          onClick={() => {
            const stake = Number(vow.stakeAmount || vow['stake-amount']) / 1000000;
            const usd = formatUSD(stake, stxPrice);
            const text = `⚔️ "${sanitize(vow.title ?? '')}" — ${stake} STX${usd ? ` (${usd})` : ''} on the line.\n\nWill they deliver? Watch it play out on @deadlockwtf 👀\n\n${window.location.href}`;
            window.open(`https://twitter.com/intent/tweet?text=${encodeURIComponent(text)}`, '_blank', 'noopener');
          }}
          className="btn-outline py-2.5 px-6 text-xs rounded-full tracking-widest font-bold border-line-strong hover:border-blue-400/50 bg-surface-raised hover:bg-blue-400/10 text-blue-400 transition-all duration-300 flex items-center gap-2"
        >
        ...
        <button
          ...
          className="btn-outline py-2.5 px-6 text-xs rounded-full tracking-widest font-bold border-line-strong hover:border-ink-muted bg-surface-raised hover:bg-surface-hover transition-all duration-300 flex items-center gap-2"
        >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
        <div className="lg:col-span-2 space-y-12">
          <section className="space-y-4">
            <div className="flex flex-wrap items-center gap-3">
              ...
              <span className="text-xs font-mono text-ink-subtle">VOW #{id}</span>
            </div>
            <h1 className="text-4xl sm:text-6xl font-bold uppercase tracking-tight leading-none text-ink break-words">{sanitize(vow.title ?? '')}</h1>
            <p className="text-lg text-gray-400 leading-relaxed max-w-3xl font-space">{sanitize(vow.description ?? '')}</p>
            <VowReactions vowId={id as string} />
          </section>
          ...
          <section className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            <div className="glass-card p-5 border-line bg-surface-raised">
              ...
              {formatUSD(Number(vow.stakeAmount || vow['stake-amount']) / 1000000, stxPrice) && (
                <div className="text-[10px] text-gray-500 mt-1">{formatUSD(Number(vow.stakeAmount || vow['stake-amount']) / 1000000, stxPrice)}</div>
              )}
            </div>
            <div className="glass-card p-5 border-line bg-surface-raised">
            ...
            <div className="glass-card p-5 border-line bg-surface-raised relative group">
          ...
          <section className="glass-card p-6 border-line bg-surface-raised flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
            ...
            {!isExpired && (
              <div className="relative">
                <button 
                  ...
                  className="px-5 py-2.5 border border-line hover:border-ink-muted text-xs font-bold tracking-widest uppercase rounded-full bg-surface-raised hover:bg-surface-hover transition-all font-bebas flex items-center gap-2"
                >
                  📅 ADD TO CALENDAR
                </button>
                  ...
                  {showCalendarMenu && (
                    <>
                      ...
                      <motion.div 
                        ...
                        exit={{ opacity: 0, y: 10 }}
                        className="absolute right-0 mt-2 w-48 bg-neutral-900 border border-line rounded-lg shadow-xl z-55 overflow-hidden font-space"
                      >
                        <a 
                          href={googleCalUrl} 
                          target="_blank" 
                          ...
                          className="block px-4 py-3 text-xs font-bold tracking-wider hover:bg-surface-raised transition-colors border-b border-line text-gray-300 hover:text-white"
                        >
                          GOOGLE CALENDAR
                        </a>
                        <button 
                          onClick={() => { downloadICS(); setShowCalendarMenu(false); }}
                          className="w-full text-left block px-4 py-3 text-xs font-bold tracking-wider hover:bg-surface-raised transition-colors text-gray-300 hover:text-white"
                        >
                          DOWNLOAD iCAL (.ICS)
                        </button>
                      </motion.div>
          ...
          {proofUrlStr && (
            <section className="glass-card p-6 border-white/15 bg-surface-raised space-y-6">
              <div className="border-b border-line pb-3 flex justify-between items-center">
                ...
                <span className="text-[10px] font-mono opacity-50 bg-surface-raised border border-line rounded px-2 py-0.5 select-all">{proofUrlStr}</span>
              </div>

              <div className="rounded-xl overflow-hidden bg-black/40 border border-line p-4 flex flex-col justify-center">
                {isTwitterProof && parsedTweetId ? (
                  ...
                  <div className="aspect-video w-full">
                    <iframe
                      ...
                      allowFullScreen
                      className="w-full h-full rounded-lg border border-line shadow-lg"
                    />
                  </div>
                ) : githubData ? (
                  <div className="space-y-4 font-space text-left">
                    <div className="flex items-center gap-3">
                      <img src={githubData.avatar} alt="Author Avatar" className="w-10 h-10 rounded-full border border-line" />
                      <div>
                    ...
                    <div className="bg-surface-raised p-4 rounded-lg border border-line">
                  ...
                  <div className="p-8 text-center space-y-4">
                    <div className="w-12 h-12 rounded-full border border-line bg-surface-raised flex items-center justify-center mx-auto text-xl">🔗</div>
                    <div>
                      <h4 className="font-bold text-white mb-1 uppercase tracking-widest text-xs">External Proof Link Provided</h4>
                      <p className="text-xs text-ink-subtle font-mono select-all truncate mb-4 max-w-md mx-auto">{proofUrlStr}</p>
                    </div>
                    <a 
                      href={proofUrlStr} 
                      target="_blank" 
                      rel="noopener noreferrer" 
                      className="px-6 py-2 border border-white/20 hover:border-line0 text-xs font-bold uppercase rounded-full tracking-widest hover:bg-surface-raised transition-all inline-block font-bebas"
                    >
                      VISIT PROOF LINK
                    </a>
                  </div>
            ...
            >
              ...
              {isChallengeClosed ? (
                <>
                  ...
                  <div className="mb-6">
                    <p className="text-[10px] text-ink-subtle uppercase tracking-widest mb-2">VOTE TIMELINE</p>
                    <VoteTimeline vowId={id as string} yesVotes={Number(vow.yesVotes || vow['yes-votes'] || 0)} noVotes={Number(vow.noVotes || vow['no-votes'] || 0)} currentBlock={currentBlock} />
                  </div>
                  <button
                ...
                <>
                  ...
                  <div className="flex gap-4">
                    <button onClick={() => handleVote(true)} disabled={votePending} className="flex-1 btn-primary bg-green-500 hover:bg-green-600 font-bold py-2.5 text-xs disabled:opacity-50 disabled:cursor-not-allowed">
                      {votePending ? 'SUBMITTING...' : 'VOTE SUCCESS'}
                    </button>
                    <button onClick={() => handleVote(false)} disabled={votePending} className="flex-1 btn-primary bg-red-500 hover:bg-red-600 font-bold py-2.5 text-xs disabled:opacity-50 disabled:cursor-not-allowed">
                      {votePending ? 'SUBMITTING...' : 'VOTE FAILURE'}
                    </button>
                  </div>
                </>
              )}
          ...
          {Number(vow.status) === VOW_STATUS.ACTIVE && !isExpired && (
            isCreator ? (
              <section className="glass-card p-8 border-white/20">
                ...
                <div className="flex gap-4">
                  <input 
                    ...
                    className="flex-grow bg-surface-raised border border-line p-3 outline-none focus:border-white transition-colors font-mono text-sm" 
                    placeholder="https://..." 
                  />
              ...
              <section className="glass-card p-8 border-line bg-surface-raised opacity-85 text-center">
        ...
        <div className="space-y-8">
          <section className="glass-card p-6 border-line bg-surface-raised">
            <h3 className="text-xl font-bold mb-6 border-b border-line pb-2">SPECTATOR POOL</h3>
            ...
            {(successPool > 0 || failurePool > 0) && (
              <div className="mb-6 h-[120px]">
                <ResponsiveContainer width="100%" height="100%">
                  <PieChart>
                    <Pie
                      data={[
                        { name: 'Success', value: successPool || 0.001 },
                        { name: 'Failure', value: failurePool || 0.001 },
                      ]}
                      cx="50%"
                      cy="50%"
                      innerRadius={32}
                      ...
                      strokeWidth={0}
                    >
                      <Cell fill="#4ade80" opacity={0.85} />
                      <Cell fill="#f87171" opacity={0.85} />
                    </Pie>
                    <Tooltip
                      contentStyle={{ background: '#0a0a0a', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 8, fontSize: 11 }}
                      formatter={(val: any) => [`${Number(val).toFixed(4)} STX`, '']}
                    />
                  </PieChart>
                </ResponsiveContainer>
              </div>
            ...
            <div className="flex flex-col gap-2 border-b border-line pb-6 mb-6">
            ...
            <div className="space-y-4">
              <div className="flex justify-between items-center">
                ...
                <div className="flex border border-line rounded-full overflow-hidden p-0.5">
                  <button 
              ...
              <div className="space-y-2">
                ...
                <input 
                  ...
                  className="w-full h-1.5 bg-surface-hover rounded-full appearance-none outline-none cursor-pointer accent-purple-500"
                />
              </div>

              <div className="space-y-2.5 bg-black/40 border border-line rounded-lg p-3 text-xs font-mono">
          ...
          <section className="glass-card p-6 border-line bg-surface-raised">
            <h3 className="text-xl font-bold mb-4 border-b border-line pb-2">PARTIES</h3>
            <div className="space-y-4">
              <div>
